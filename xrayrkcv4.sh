#!/bin/bash

# ==========================================
# KHAI BÁO MÀU SẮC
# ==========================================
YELLOW='\033[1;33m'
WHITE='\033[1;37m'
PURPLE='\033[1;35m'
CYAN='\033[1;36m'
GREEN='\033[1;32m'
RED='\033[1;31m'
NC='\033[0m'

clear
echo -e "${YELLOW}==> Đang kiểm tra hệ thống và IP <==${NC}"
PUBLIC_IP=$(curl -s https://api.ipify.org || curl -s https://ipv4.icanhazip.com)
[[ -z "$PUBLIC_IP" ]] && PUBLIC_IP="127.0.0.1"

# ==========================================
# THÔNG TIN CẤU HÌNH API (ĐÃ ĐIỀN KEY)
# ==========================================
set_api_info() {
    CUR_API_HOST="https://kingcloud.click"
    CUR_API_KEY="fdsf326s2vchrt66gb656467hgfjfyjyilmh4902du15iu" 
    
    # Reality Config
    CUR_REALITY_KEY="LVfcz9sfL8Gr-UkAWQYsWpnsRCCgvE3NpntcwHNYDbo"
    CUR_SID="63856d5c"
}

vmess_nodes=()
trojan_nodes=()
ss_nodes=()
vless_nodes=()

# --- HÀM NHẬP NODE ID ---
ask_vmess() {
    echo -e -n "${WHITE}Số lượng node ${PURPLE}VMess${WHITE}: ${CYAN}"; read count
    for ((i=1; i<=count; i++)); do
        echo -e -n "${NC} ↳ Nhập Node ID $i: ${CYAN}"; read id
        [[ -n "$id" ]] && vmess_nodes+=("$id")
    done
}

ask_trojan() {
    echo -e -n "${WHITE}Số lượng node ${YELLOW}Trojan${WHITE}: ${CYAN}"; read count
    for ((i=1; i<=count; i++)); do
        echo -e -n "${NC} ↳ Nhập Node ID $i: ${CYAN}"; read id
        [[ -n "$id" ]] && trojan_nodes+=("$id")
    done
}

ask_ss() {
    echo -e -n "${WHITE}Số lượng node ${GREEN}Shadowsocks${WHITE}: ${CYAN}"; read count
    for ((i=1; i<=count; i++)); do
        echo -e -n "${NC} ↳ Nhập Node ID $i: ${CYAN}"; read id
        [[ -n "$id" ]] && ss_nodes+=("$id")
    done
}

ask_vless() {
    echo -e -n "${WHITE}Số lượng node ${PURPLE}VLESS (Reality)${WHITE}: ${CYAN}"; read count
    for ((i=1; i<=count; i++)); do
        echo -e -n "${NC} ↳ Nhập Node ID $i: ${CYAN}"; read id
        echo -e -n "${NC} ↳ Nhập SNI (Mặc định learn.microsoft.com): ${CYAN}"; read sni
        [[ -z "$sni" ]] && sni="learn.microsoft.com"
        [[ -n "$id" ]] && vless_nodes+=("$id|$sni")
    done
}

# ==========================================
# MENU LỰA CHỌN
# ==========================================
while true; do
    vmess_nodes=(); trojan_nodes=(); ss_nodes=(); vless_nodes=()
    echo -e "${CYAN}======================================${NC}"
    echo -e "${WHITE}CÀI ĐẶT XRAYR (NGUỒN VAHIRU)${NC}"
    echo -e "${WHITE}IP Máy Chủ: ${YELLOW}${PUBLIC_IP}${NC}"
    echo -e "${PURPLE}  1.${WHITE} Trojan           ${PURPLE}  2.${WHITE} VMess"
    echo -e "${PURPLE}  3.${WHITE} Shadowsocks      ${PURPLE}  4.${WHITE} VLESS (Reality)"
    echo -e "${PURPLE}  5.${WHITE} Combo (VMess + Trojan + VLESS)"
    echo -e "${CYAN}======================================${NC}"
    echo -e -n "Nhập lựa chọn (1-5): "; read choice

    case $choice in
        1) ask_trojan ;;
        2) ask_vmess ;;
        3) ask_ss ;;
        4) ask_vless ;;
        5) ask_vmess; ask_trojan; ask_vless ;;
        *) echo -e "${RED}Lựa chọn không hợp lệ!${NC}"; continue ;;
    esac

    echo -e -n "${GREEN}Xác nhận cài đặt? (y/n): ${NC}"; read check
    [[ "$check" == "y" || "$check" == "Y" ]] && break
    clear
done

# ==========================================
# CÀI ĐẶT CORE VÀ TẠO CHỨNG CHỈ
# ==========================================
echo -e "${YELLOW}==> Đang tải và cài đặt XrayR Core...${NC}"
wget -N https://raw.githubusercontent.com/vahiru/XrayR-release/master/install.sh && bash install.sh

mkdir -p /etc/XrayR

# Tạo File Chứng Chỉ Giả (Dành cho Trojan)
cat > /etc/XrayR/viefast.crt <<EOF
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOF

cat > /etc/XrayR/viefast.key <<EOF
-----BEGIN PRIVATE KEY-----

aGU6njQK0ycNa80Gmy0C9G1B
-----END PRIVATE KEY-----
EOF

# ==========================================
# KHỞI TẠO FILE CONFIG.YML
# ==========================================
echo -e "${YELLOW}==> Đang biên dịch file config.yml...${NC}"
set_api_info

cat > /etc/XrayR/config.yml <<EOF
Log:
  Level: none
  AccessPath: # /etc/XrayR/access.Log
  ErrorPath: # /etc/XrayR/error.log
DnsConfigPath: # /etc/XrayR/dns.json
RouteConfigPath: # /etc/XrayR/route.json
InboundConfigPath: # /etc/XrayR/custom_inbound.json
OutboundConfigPath: # /etc/XrayR/custom_outbound.json
ConnectionConfig:
  Handshake: 4
  ConnIdle: 30
  UplinkOnly: 2
  DownlinkOnly: 4
  BufferSize: 64
Nodes:
EOF

# -- Render VMess Nodes --
for id in "${vmess_nodes[@]}"; do
cat >> /etc/XrayR/config.yml <<EOF
  - PanelType: "NewV2board"
    ApiConfig:
      ApiHost: "${CUR_API_HOST}"
      ApiKey: "${CUR_API_KEY}"
      NodeID: ${id}
      NodeType: V2ray
      Timeout: 30
      EnableVless: false
      EnableXTLS: false
    ControllerConfig:
      DisableSniffing: True
      ListenIP: 0.0.0.0
      UpdatePeriodic: 60
      CertConfig:
        CertMode: none
        CertDomain: "${PUBLIC_IP}"
EOF
done

# -- Render Trojan Nodes --
for id in "${trojan_nodes[@]}"; do
cat >> /etc/XrayR/config.yml <<EOF
  - PanelType: "NewV2board"
    ApiConfig:
      ApiHost: "${CUR_API_HOST}"
      ApiKey: "${CUR_API_KEY}"
      NodeID: ${id}
      NodeType: Trojan
      Timeout: 30
    ControllerConfig:
      DisableSniffing: True
      ListenIP: 0.0.0.0
      UpdatePeriodic: 60
      CertConfig:
        CertMode: file
        CertDomain: "${PUBLIC_IP}"
        CertFile: /etc/XrayR/viefast.crt
        KeyFile: /etc/XrayR/viefast.key
EOF
done

# -- Render Shadowsocks Nodes --
for id in "${ss_nodes[@]}"; do
cat >> /etc/XrayR/config.yml <<EOF
  - PanelType: "NewV2board"
    ApiConfig:
      ApiHost: "${CUR_API_HOST}"
      ApiKey: "${CUR_API_KEY}"
      NodeID: ${id}
      NodeType: Shadowsocks
      Timeout: 30
    ControllerConfig:
      DisableSniffing: True
      ListenIP: 0.0.0.0
      UpdatePeriodic: 60
      CertConfig:
        CertMode: none
EOF
done

# -- Render VLESS Reality Nodes --
for item in "${vless_nodes[@]}"; do
    id=$(echo "$item" | cut -d'|' -f1)
    sni=$(echo "$item" | cut -d'|' -f2)
cat >> /etc/XrayR/config.yml <<EOF
  - PanelType: "NewV2board"
    ApiConfig:
      ApiHost: "${CUR_API_HOST}"
      ApiKey: "${CUR_API_KEY}"
      NodeID: ${id}
      NodeType: V2ray
      EnableVless: true
      Timeout: 30
    ControllerConfig:
      DisableSniffing: true
      ListenIP: 0.0.0.0
      UpdatePeriodic: 60
      EnableREALITY: true
      REALITYConfigs:
        Show: false
        Dest: "${sni}:443"
        ProxyProtocolVer: 0
        ServerNames: ["${sni}"]
        PrivateKey: "${CUR_REALITY_KEY}"
        ShortIds: ["${CUR_SID}"]
      CertConfig:
        CertMode: none
EOF
done

# ==========================================
# KHỞI ĐỘNG DỊCH VỤ
# ==========================================
echo -e "${GREEN}==> Đang khởi động lại XrayR...${NC}"
xrayr restart

clear
echo -e "${CYAN}====================================================${NC}"
echo -e "${GREEN}🎉 CÀI ĐẶT HOÀN TẤT!${NC}"
echo -e "${WHITE}Kiểm tra trạng thái: ${YELLOW}xrayr status${NC}"
echo -e "${WHITE}Xem log lỗi: ${YELLOW}xrayr log${NC}"
echo -e "${CYAN}====================================================${NC}"
